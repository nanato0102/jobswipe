      ...
      <CompanyMobileTabs>
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8 space-y-6 w-full">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {/* 残りオファー枠 */}
            <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-1">
            ...
            <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-1">
              ...
              <p className="text-xs text-slate-400 pt-1">スワイプで保存した学生</p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-1">
              ...
              <p className="text-xs text-emerald-700 font-semibold pt-1">承諾率 60%</p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-1">
              ...
              <p className="text-xs text-slate-400 pt-1">今月チェックした学生</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-5 sm:p-6 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-base font-bold text-slate-900">{stats.planName}</h2>
                </div>
                <p className="text-xs sm:text-sm text-slate-500 mt-1">
                  月間オファー枠: <span className="font-semibold text-slate-900">{stats.monthlyQuota}枠</span> / 次回更新日: <span className="font-semibold text-slate-900">{stats.nextResetDate}</span>
                </p>
              </div>

              <Link
                href="/company/plans"
                className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-md text-sm font-semibold transition-all shadow-xs flex items-center justify-center gap-1.5 whitespace-nowrap self-start sm:self-auto cursor-pointer"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-sm text-slate-500">
              <p>プランのアップグレードやスポットでの追加増枠（+20枠〜）は専用ページよりお申し込みいただけます。</p>
              <Link
                href="/tokusho"
                target="_blank"
                className="text-xs font-semibold text-slate-500 hover:text-slate-800 hover:underline inline-flex items-center gap-1 whitespace-nowrap"
              >
          ...
          <div className="bg-white rounded-xl border border-slate-200/90 shadow-xs p-5 sm:p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <Link
                href="/company/chat"
                className="text-xs sm:text-sm font-semibold text-blue-700 hover:underline flex items-center gap-1"
              >
                <span>チャット一覧へ →</span>
              </Link>
            </div>
            ...
            <div className="divide-y divide-slate-100">
              {offers.map((offer) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 self-end sm:self-auto flex-shrink-0">
                    <span className="text-xs text-slate-400">{offer.createdAt}</span>
                    {offer.status === "ACCEPTED" && (
                      <Link
              ...
              {offers.length === 0 && (
                <p className="py-6 text-center text-xs sm:text-sm text-slate-400">オファー送信履歴はありません</p>
              )}
            </div>
          </div>
        </div>
